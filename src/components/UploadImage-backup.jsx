...
const UploadImage = ({ state, setUploadedPhotos }) => {
  const {
    listing: { photos: storePhotos },
    localRemovedPhotos,
  } = useSelector((store) => store.singleListing)
  const { user } = useSelector((store) => store.user)

  // SINCE I NEED TO PASS SET METHOD AS PARAM SO I NEED TO CREATE A LOCAL STATE
  ...
  useEffect(() => {
    ...
    // dispatch(uploadPhotoToLocal(photos))
  }, [photos])

  // CAN'T USE CODE BELOW SINCE WILL CREATE INFINITE LOOP
  // when remove photo from redux update local photos
  // useEffect(() => {
